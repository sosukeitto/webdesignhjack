...
import { categories, getDifficultyLabel } from "@/lib/types";
    ...
    <Link href={`/tips/${tip.slug}`} className={styles.card}>
      <div className={styles.header}>
        ...
        <div className={styles.badges}>
          <span className={styles.difficulty}>{getDifficultyLabel(tip.difficulty)}</span>
          {priority === "high" && (
            <span className={styles.priorityHigh}>🔥 必須</span>
          )}
        </div>
      </div>

      <h3 className={styles.title}>
        <span className={styles.hilight}>{tip.title}</span>
      </h3>
